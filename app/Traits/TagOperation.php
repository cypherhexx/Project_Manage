...
trait TagOperation {
    ...
    function tags()
    {
        ...
        ->where('taggable_type', self::class )->withPivot('taggable_type');
    }
    ...
    function tag_attach($tag_ids)
    {
        if($tag_ids && count($tag_ids) > 0)
        {
            ...
            {
                $data[$tag_id] = ['taggable_type' => self::class ];
            }
    ...
    function tag_sync($tag_ids)
    {
        if($tag_ids && count($tag_ids) > 0)
        {
            ...
            $this->tags()->wherePivot('taggable_type' , self::class )->sync($data);
        }
        else
        {
            $this->tags()->wherePivot('taggable_type' , self::class )->sync([]);
        }
    }
